
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
 * This is a barrel export file for all models and their related types.
 *
 * 🟢 You can import this file directly.
 */
export type * from './models/User.js'
export type * from './models/Profile.js'
export type * from './models/ConnectRequest.js'
export type * from './models/Course.js'
export type * from './models/SemesterMapping.js'
export type * from './models/Elective.js'
export type * from './models/Resource.js'
export type * from './models/PlacementDrive.js'
export type * from './models/PlacementOffer.js'
export type * from './models/DriveResource.js'
export type * from './models/Company.js'
export type * from './models/InterviewExperience.js'
export type * from './models/InterviewRound.js'
export type * from './models/Question.js'
export type * from './commonInputTypes.js'